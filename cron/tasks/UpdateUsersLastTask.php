 ...
 * @version 1.0
 */
class UpdateUsersLastTask extends Moodspin_Cron_Task implements Moodspin_Cron_Task_Interface 
{
    protected $_db;
    private $_count;
    ...
    {
        $this->_dropTmpTable();
        $lastDate = date('Y-m-d H:i:s', time() - 3 * 24 * 3600);
                
        $query = "CREATE TABLE `temp_twitter_users` SELECT twitter_id FROM report_users WHERE (NOT followers_num OR NOT following_num ) AND (latest_status_date > '".$lastDate."') AND creation_date IS NULL;";
